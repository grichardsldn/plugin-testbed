...
def  main():
  ...
  installer = "\installer\Testbench Installer.exe"
   
  if demo:
    installer = "\installer\Testbench Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Testbench manual.pdf" 
  ]
  ...
  ZIPNAME = "Testbench-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Testbench-v" + FULLVERSIONSTR + "-win-demo.zip"
